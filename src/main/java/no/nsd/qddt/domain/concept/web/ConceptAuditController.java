...
public class ConceptAuditController {
    ...
    public Page<Revision<Integer, Concept>> allProjects(
         ...
         return auditService.findRevisionsByChangeKindNotIn(id,changekinds, pageable);
    }

    // @JsonView(View.Audit.class)
    @RequestMapping(value = "/{id}/allinclatest", method = RequestMethod.GET)
    public Page<Revision<Integer, Concept>> allIncludinglatest(
            @PathVariable("id") UUID id,
        ...
        return auditService.findRevisionsByChangeKindIncludeLatest(id,changekinds, pageable);
    }
